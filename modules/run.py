...
class Run:
    ...
    def explore(self, env, normalizer, policy, direction=None, delta=None, eval=False):
        ...
        while not done and num_plays < self.hp.episode_length:
            ...
            num_plays += 1
            self.total_steps +=1
            if self.hp.render_train:
                self.env.render()
            if eval and self.hp.render_eval and not self.hp.render_train:
                self.env.render()
        return sum_rewards
    ...
    def train(self):
        for episode in range(self.hp.nb_episodes):
            ...
            avg_reward_evaluation = self.explore(self.env, self.normalizer, self.policy, direction=None, eval=True)
            self.evaluations.append(
                (avg_reward_evaluation, episode, self.total_steps, time.time())
            )

            if self.hp.save_freq and episode % self.hp.save_freq == 0:
                self.policy.save()
